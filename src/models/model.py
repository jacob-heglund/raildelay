...
class OutputLayer(nn.Module):
    ...
    def forward(self, X):
        """
        Args:
            X (Torch.Tensor): X shape  = (batch_size, n_timesteps_in, n_nodes, n_channels)
        Returns:
            Torch.Tensor: out shape  = (batch_size, n_timesteps_out, n_nodes, n_features_out)
        """
...
class STConvBlock(nn.Module):
    ...
    def forward(self, X, graph_kernel):
        """
        Args:
            X (Torch.Tensor): input data, X shape  = (batch_size, n_timesteps_in, n_nodes, c_in)
            graph_kernel (Torch.Tensor): Chebyshev or 1st order approximation of scaled graph Laplacien, graph_kernel shape  = (n_nodes, ks * n_nodes)
...
#TODO
#DONE make input to STGCN X.shape = (batch_size, c_in, n_timesteps_in, n_nodes)
# update docs to reflect this change

class STGCN(nn.Module):
    def __init__(self, blocks, n_timesteps_in, n_timesteps_out, n_nodes, device, inf_mode, ks=5, kt=3, drop_prob=0.0):
        ...
        if inf_mode == "individual":
            ...
            self.output_layer = OutputLayer(blocks[2], n_timesteps_in, n_timesteps_out)
    ...
    def forward(self, X, graph_kernel):
        ...
        Torch.Tensor: model prediction
            if inf_mode == "individual":
                y_hat shape  = (batch_size, 1, n_nodes, n_features_out)
            elif inf_mode == "multiple"
                y_hat shape  = (batch_size, n_timesteps_future, n_nodes, n_features_out)
        """
